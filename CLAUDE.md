...
# Test the endpoint
curl -X POST http://localhost:8000/predict -H "Content-Type: application/json" -d '{"patient_id": 1, "correct_key_objects": 4, "correct_secondary_objects": 5, "incorrect_objects": 1, "total_key_objects": 5, "total_secondary_objects": 8, "total_events": 5, "correct_events": 4, "comprehension_score": 2, "response_times": [2.1, 3.5, 1.8, 2.0, 2.5, 1.9, 2.3, 2.1, 2.0, 1.7], "total_questions": 10, "incorrect_answers": 2}'
```
...
3. `SessionMetrics.from_raw()` computes the 6 cognitive metrics (ORS uses the new key/secondary weighted formula)
...
5. `feature_vector(metrics, context)` builds the 15-feature vector
...
- Clinical traceability: every prediction persists the 15 input features and the 3 class probabilities so any decision can be explained
